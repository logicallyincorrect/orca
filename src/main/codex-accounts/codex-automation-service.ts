...
export class CodexAutomationService {
  ...
  async isExhausted(home: string, signal: AbortSignal): Promise<boolean> {
    const account = this.store
      .getSettings()
      .codexManagedAccounts.find((entry) => entry.managedHomePath === home)
    if ((!account && home !== getSystemCodexHomePath()) || this.stopped || signal.aborted) {
      return false
    }
    const usage = account
      ? await this.readUsage(account, signal)
      : await fetchCodexRateLimits({ codexHomePath: home, signal })
    return !signal.aborted && codexQuotaAvailability(usage, Date.now()) === 'exhausted'
  }
  ...
  async findReplacement(
    ...
    if (!outgoing && home !== getSystemCodexHomePath()) {
      return null
    }
    const target = outgoing
      ? getCodexSelectionTargetForAccount(outgoing)
      : { runtime: 'host' as const }
    const selectedId = getSelectedCodexAccountIdForTarget(settings, target)
    if (!allowUnselectedSource && selectedId !== (outgoing?.id ?? null)) {
      return null
    }
    let nextResetAt: number | null = null
    const candidates = [...settings.codexManagedAccounts].sort(
