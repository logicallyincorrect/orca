...
export async function failoverCodexAccount(
  ...
  return deps.serialize(async () => {
    ...
    if (
      !isCurrent() ||
      !input.isSafe() ||
      (!outgoing && input.home !== getSystemCodexHomePath()) ||
      getCodexSelectionLaneKey(getCodexSelectionTargetForAccount(candidate)) !==
        (outgoing
          ? getCodexSelectionLaneKey(getCodexSelectionTargetForAccount(outgoing))
          : 'host') ||
      !settings.codexManagedAccounts.some(
        (entry) => entry.id === candidate.id && entry.managedHomePath === candidate.managedHomePath
      ) ||
      (!input.allowUnselectedSource &&
        getSelectedCodexAccountIdForTarget(
          settings,
          outgoing ? getCodexSelectionTargetForAccount(outgoing) : { runtime: 'host' }
        ) !== (outgoing?.id ?? null))
    ) {
      return false
    }
