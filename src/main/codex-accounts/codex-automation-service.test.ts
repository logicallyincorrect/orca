...
describe('Codex account eligibility', () => {
  it('prefers the selected account when a terminal is still running under the old one', async () => {
    const f = fixture()
    f.settings.activeCodexManagedAccountIdsByRuntime = { host: 'c', wsl: {} }
    expect((await f.service.findReplacement('/a', f.signal, [], true))?.id).toBe('c')
    expect(fetchUsage.mock.calls.map(([input]) => input.codexHomePath)).toEqual(['/c'])
  })
  it('only confirms exhaustion from a fresh successful quota read', async () => {
    const f = fixture()
    fetchUsage.mockResolvedValueOnce(usage(100))
    expect(await f.service.isExhausted('/a', f.signal)).toBe(true)
    fetchUsage.mockResolvedValueOnce({ ...usage(100), updatedAt: 1 })
    expect(await f.service.isExhausted('/a', f.signal)).toBe(false)
    expect(await f.service.isExhausted('/unknown', f.signal)).toBe(false)
  })
...
it('reads System default quota and selects capacity from host accounts', async () => {
  const f = fixture()
  fetchUsage.mockResolvedValueOnce(usage(100))
  expect(await f.service.isExhausted(getSystemCodexHomePath(), f.signal)).toBe(true)
  expect(fetchUsage).toHaveBeenLastCalledWith({
    codexHomePath: getSystemCodexHomePath(),
    signal: f.signal
  })
  expect(await f.service.findReplacement(getSystemCodexHomePath(), f.signal)).toBeNull()
  expect((await f.service.findReplacement(getSystemCodexHomePath(), f.signal, [], true))?.id).toBe(
    'a'
  )
})
